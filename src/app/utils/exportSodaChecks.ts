...
export function generateSodaChecksYAML(
    ...
    for (const col of results.column_profiles) {
        ...
        if (validCount > 0 && distinct >= validCount * 0.999) { // Practically unique
        ...
        if (col.numeric_stats) {
            ...
            const minThreshold = stats.min >= 0
                ? stats.min * (1 - toleranceMultiplier)
                : stats.min * (1 + toleranceMultiplier);
            const maxThreshold = stats.max >= 0
