 ...
 */
type ColumnProfile = ProfileResult['column_profiles'][number];

function validateType(
  column: ColumnProfile,
  schemaType: string | string[],
): { status: 'pass' | 'fail'; observed?: string; expected?: string; reason?: string } {
  ...
  for (const type of allowedTypes) {
    if (type === 'string') {
      if (inferredType === 'string' || inferredType === 'date' || inferredType === 'datetime')
        isMatch = true;
    } else if (type === 'integer') {
      if (inferredType === 'integer') isMatch = true;
    } else if (type === 'number') {
      if (inferredType === 'integer' || inferredType === 'float' || inferredType === 'numeric')
        isMatch = true;
