 ...
 *
 * Generates a valid JSON Schema document from DataLens profile results.
 ...
 */
function mapDataLensTypeToJsonSchema(
  inferredType: string
...
export function generateJsonSchema(results: ProfileResult, filename: string): JsonSchema {
  ...
  for (const col of results.column_profiles) {
    ...
    const { type, format } = mapDataLensTypeToJsonSchema(inferred_type);
