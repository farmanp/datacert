        ...
        >
          ...
          <div class="px-8 py-6 border-b border-slate-700 bg-slate-800/50 flex items-center justify-between">
            <div>
              ...
              <p class="text-slate-400 text-xs mt-1 uppercase tracking-widest font-bold">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2">
              ...
              <For each={FORMAT_OPTIONS}>
                {(option) => (
                  <button
                    ...
                    class={`w-full text-left p-4 rounded-2xl transition-all group flex items-start gap-4 border ${
                  ...
                  >
                    ...
                    <div>
                      <p
                        class={`text-sm font-bold ${selectedFormat() === option.id ? 'text-white' : 'text-slate-200'}`}
                      >
                        {option.label}
                      </p>
                      <p
                        class={`text-[11px] leading-relaxed mt-1 ${selectedFormat() === option.id ? 'text-blue-100' : 'text-slate-400'}`}
                      >
                        {option.description}
                      </p>
                    </div>
            ...
            <div class="p-8 bg-slate-900/30 flex flex-col justify-between">
              <div class="space-y-8">
                ...
                <Show when={currentOption().requiresTolerance}>
                  <div class="space-y-4">
                    <div class="flex items-center justify-between">
                      ...
                      <span class="text-sm font-black text-blue-400 bg-blue-400/10 px-2 py-0.5 rounded-lg">
                        {tolerance()}%
                      </span>
                    </div>
                    <input
                    ...
                    <div class="p-4 bg-slate-800/50 rounded-2xl border border-slate-700/50">
                      <p class="text-[10px] uppercase font-black text-slate-500 tracking-widest mb-2">
                        Preview Logic
                      </p>
                ...
                <Show
                  when={!currentOption().requiresTolerance && !currentOption().requiresTableName}
                >
                  <div class="h-full flex flex-col items-center justify-center text-center space-y-4 py-8">
                    <div
                      class={`p-4 rounded-3xl bg-slate-800 border border-slate-700/50 ${currentOption().iconClass}`}
                    >
                      <svg class="w-12 h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path
                          d={currentOption().icon}
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                    ...
                    <div>
                      ...
                      <p class="text-[11px] text-slate-500 mt-2 max-w-[200px]">
                        No additional configuration required for this format.
                      </p>
                    </div>
                  </div>
                </Show>
                ...
                >
                  ...
                  {isExporting()
                    ...
                    : selectedFormat() === 'markdown'
                      ? 'Copy to Clipboard'
                      : `Download .${currentOption().extension}`}
                </button>
