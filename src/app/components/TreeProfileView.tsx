    ...
    try {
      ...
      if (isParquet) {
        ...
        if (trimmed.startsWith('[')) {
          // JSON array format
          jsonData = JSON.parse(jsonText) as JsonValue[];
        } else {
          // JSON Lines format
          jsonData = trimmed
            .split('\n')
        ...
        const extractValue = (obj: JsonValue, path: string): JsonValue | null => {
        ...
        const headers = selectedPaths.map((path) => path.replace(/^\$\./, ''));
        const rows: Record<string, JsonValue | null>[] = jsonData.map((row) => {
          const newRow: Record<string, JsonValue | null> = {};
          ...
          <Show when={isAnalyzing()}>
            <div class="flex items-center justify-center py-12">
              <div class="text-center">
                ...
                <p class="text-slate-600 dark:text-slate-400">Analyzing JSON structure...</p>
              </div>
            </div>
          </Show>
