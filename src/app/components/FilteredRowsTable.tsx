      ...
      <table class="w-full text-sm text-left text-slate-500 dark:text-slate-400">
        <thead class="text-xs text-slate-700 uppercase bg-slate-50 dark:bg-slate-800 dark:text-slate-400 sticky top-0">
          <tr>
            ...
            <For each={store.drilldown.headers}>
              {(header) => (
                <th
                  class={`px-3 py-2 border-b dark:border-slate-700 whitespace-nowrap ${
                    header === store.drilldown.columnName
        ...
        <tbody>
            ...
            {(item) => (
              <tr class="bg-white border-b dark:bg-slate-900 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                ...
                <For each={item.row}>
                  {(cell, i) => (
                    <td
                      class={`px-3 py-2 whitespace-nowrap max-w-[300px] overflow-hidden text-ellipsis ${
                        i() === highlightIndex()
          ...
          {store.drilldown.currentRows.length === 0 && !store.drilldown.isLoading && (
            <tr>
              <td
                colspan={store.drilldown.headers.length + 1}
