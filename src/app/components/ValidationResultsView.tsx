                    ...
                    <div class="bg-slate-800 rounded-3xl border border-slate-700 overflow-hidden shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-300">
                        ...
                        <div class="px-6 py-4 border-b border-slate-700 bg-slate-800/50 flex items-center justify-between">
                            <div>
                                <h4 class="text-sm font-black text-white flex items-center gap-2">
                                    <span class={`p-1.5 rounded-lg ${summary.format === 'gx' ? 'bg-yellow-500/10 text-yellow-400' :
                                        summary.format === 'soda' ? 'bg-cyan-500/10 text-cyan-400' :
                        ...
                        <div class="grid grid-cols-3 divide-x divide-slate-700 border-b border-slate-700 bg-slate-900/40">
                            <div class="px-4 py-3 text-center">
                                <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest">Passed</p>
                                <p class="text-lg font-bold text-emerald-400 tabular-nums">{summary.passed}</p>
                            </div>
                            <div class="px-4 py-3 text-center">
                                <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest">Failed</p>
                                <p class="text-lg font-bold text-rose-400 tabular-nums">{summary.failed}</p>
                            </div>
                            <div class="px-4 py-3 text-center">
                                <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest">Skipped</p>
...
const ValidationResultItem: Component<{ result: ValidationResult }> = (props) => {
    const statusColor = () => {
        switch (props.result.status) {
            case 'pass': return 'bg-emerald-500/10 text-emerald-400 ring-emerald-500/20';
            case 'fail': return 'bg-rose-500/10 text-rose-400 ring-rose-500/20';
            case 'skipped': return 'bg-slate-500/10 text-slate-400 ring-slate-500/20';
        }
    };
    ...
    const statusIcon = () => {
        switch (props.result.status) {
            case 'pass':
                return <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" /></svg>;
            case 'fail':
                return <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12" /></svg>;
            case 'skipped':
                return <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
        }
    };

    return (
        <div class="p-4 flex gap-4 transition-colors hover:bg-slate-700/20 group">
            <div class={`mt-1 h-6 w-6 rounded-full flex items-center justify-center ring-1 shrink-0 ${statusColor()}`}>
                {statusIcon()}
            </div>
