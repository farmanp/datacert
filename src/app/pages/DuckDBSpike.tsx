...
const DuckDBSpike: Component = () => {
  ...
  const testLargeParquet = async () => {
    ...
    try {
      ...
      const startTime = performance.now();
      await conn.query(
        `CREATE TABLE large_table AS SELECT range as id, random() as val FROM range(1000000)`,
      );
      const endTime = performance.now();
      ...
      const res = await conn.query(
        `SELECT avg(val) as average, count(*) as count FROM large_table`,
      );
  ...
  return (
    <div class="p-8 bg-zinc-950 text-zinc-100 min-h-screen font-sans">
      ...
      <div class="mb-4">
        Status:{' '}
        <span class={status() === 'Ready' ? 'text-green-400' : 'text-yellow-400'}>{status()}</span>
      </div>
      ...
      <div class="mt-8 text-sm text-zinc-500">
        <p>
          Bundle Info: Using JSDelivr CDN for Wasm/Worker assets to avoid Vite setup hurdles for
          spike.
        </p>
