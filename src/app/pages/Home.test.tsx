...
vi.mock('@solidjs/router', () => ({
  ...
  Router: (props: ParentProps) => <>{props.children}</>,
}));
...
describe('Home Page Drag & Drop', () => {
  ...
  it('shows the global drag overlay when dragging a file', async () => {
    ...
    const dragData: DataTransferStub = {
      types: ['Files'],
      items: [{ kind: 'file' }],
    };
  ...
  it('hides the overlay when drag leaves the window', async () => {
    ...
    Object.defineProperty(leaveEvent, 'clientX', { value: -1 }); // Outside window
  ...
  it('displays an error if an invalid file type is dropped', async () => {
    ...
    const dropData: DataTransferStub = {
      files: [mockFile],
      types: ['Files'],
    };
