...
**Question:**
What is the exact JSON structure of a Great Expectations Expectation Suite, and which expectations can we reliably generate from DataCert ProfileResult data?

**Context:**
DataCert users want to export their profiling results as Great Expectations suites to integrate with existing GX validation pipelines. We need to understand the exact format to generate valid, importable suites.
...
- Mapping DataCert metrics to GX expectations
...
- [ ] Create mapping table: DataCert metrics → GX expectations
...
4. Map DataCert ProfileResult fields to expectation kwargs
...
**DataCert → GX Expectation Mapping:**

| DataCert Metric | GX Expectation | Notes |
|-----------------|----------------|-------|
| nullCount / totalRows | `expect_column_values_to_not_be_null` | Use `mostly` param with tolerance |
| inferredType | `expect_column_values_to_be_of_type` | Map DataCert types to GX types |
